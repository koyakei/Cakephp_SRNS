...
class TagsController extends AppController {
	...
	public function formAdd() {
		$inserted_id = $this->Common->singleAdd ( $this->request->query ( 'name' ), $this->Auth->user ( "id" ) );
		$this->Common->nestedAdd ( $this, $this->request->query ( 'root_ids' ), $this->request->query ( 'trikey_ids' ), $this->request->query ( 'parent_ids' ), $inserted_id, $this->request->query ( 'quantize_id' ) );
		$this->LInk->tickStreambyid ( "Article" );
		$this->Tag->follow_unfollow ( $inserted_id, true );
		$this->set ( "added_entity", $Article->find ( 'all', (array (
				'condition' => array (
	...
	public function get_child_node($id, $base_trikey, $model) {
		$trikey = $this->Trikey_list->find ( 'all', array (
				'conditions' => array (
						"Trikey_list.id" => $id
				)
		) );
		...
		foreach ( $trikeys as $trikey ) {
			$conditionsSubQuery ['AND'] = array (
					'"' . $model . '"."link_LFrom" ' => $id,
					'"' . $model . '"."trikey_id" =' . $trikey ["Trikey_list"] ["LFrom"]
			);
			$subQuery = $db_Base_trikey_tag->buildStatement ( array (
					'fields' => array (
							'"Trikey_list"."LFrom"'
					),
	...
	private function get_grandSon($parent_node, &$model, &$base_id, &$base_trikey) {
		foreach ( $parent_node as $iterator => $child_result ) {
			$parent_node [$iterator] ["child_node"] = $this->get_child_each_model ( $child_result ["$model"] ["ID"], $base_id, $base_trikey );
		}
		return $parent_node;
	}
	...
	private function reply_node_cutter($non_bace_node, $bace_node, $base_trikey) {
		...
		$trikey_list = $this->Trikey_list->find ( 'all', array (
				'condition' => array (
						'id' => $id,
						'NOT' => array (
								'LFrom' => $base_trikey
	...
	public function exchange($id = NULL) {
		...
		if ($this->request->is ( array (
			...
			$targetTagauthcounts = $this->Tagauth->find ( 'first', array (
					'conditions' => array (
							'Tagauthcount' => array (
									'tag_id' => $id,
	...
	public function notice($ids, $notice_contents) {
		foreach ( $ids as $id ) {
			...
			foreach ( $target_user_ids as $target_user_id ) {
				$massage = "cake/" + $this->ATSwicher ( $id ) + "/" + $id + "" + "更新されました" + "<br>" + $notice_contents;
	...
	public function anonymous_view($id = null) {
		...
		foreach ( $key as $key => $value ) {
			...
			$tableresults [$i] = array (
					'ID' => $key,
					'name' => $value,
					'head' => $this->taghash,
					'tag' => $this->articleparentres,
					'article' => $this->tagparentres
			);
			$i ++;
		}
		$this->set ( 'tableresults', $tableresults );
		$this->set ( 'ulist', $this->User->find ( 'list', array (
				'fields' => array (
						'ID',
						'username'
				)
	...
	public function transmitter($leftID = null, $leftKeyID = null, $rightID = null, $rightKeyID = null) {
		...
		$this->loadModel ( 'Article' );
		if ($rightID == null) {
			$newart = $this->Article->find ( 'all', array (
					'order' => array (
...
$newart );
			...
			if ($rightID != null or $rightID != 0) {
				...
				if ($rightID <= 100000) {
					$options = array (
							'conditions' => array (
									'Taguser.' . $this->Taguser->primaryKey => $rightID
							),
							'order' => array (
									'Taguser.ID'
							)
					...
					$options = array (
							'conditions' => array (
									'Article.' . $this->Article->primaryKey => $rightID
							),
							'order' => array (
									'Article.ID'
							)
		...
		if ($leftID != null and $leftID != 0) {
			...
			if ($leftID < 100000) {
				$options = array (
						'conditions' => array (
								'Taguser.' . $this->Taguser->primaryKey => $leftID
						),
						'order' => array (
								'Taguser.ID'
						)
				...
				$options = array (
						'conditions' => array (
								'Article.' . $this->Article->primaryKey => $leftID
						),
						'order' => array (
								'Article.ID'
						)
		...
		$this->loadModel ( 'Key' );
		$this->loadModel ( 'User' );
		$this->set ( 'keylist', $this->Key->find ( 'list', array (
				'fields' => array (
						'ID',
						'name'
				)
...
;
	...
	function replysingle() {
		$options = array (
				'key' => Configure::read ( 'tagID.reply' )
		);
		debug ( $this->Common->trifinderbyidAndSet ( $this, array (
				1
		), $options ) );
	}
	...
	public function GET_all_search() {
		...
		$tableresults = array ();
		$sorting_tags = array (
				$this->request->query ( 'sorting_tags' )
		);
		$taghash = array ();
		$id = $this->request->query ( 'id' );
	...
	public function GET_search($andSet_ids, $trikey = null, $sorting_tags, &$taghash) {
		$temp = $this->Common->trifinderbyidAndSet ( $this, $andSet_ids, array (
				'key' => $trikey
		) );
